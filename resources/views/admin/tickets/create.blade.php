...
<div class="card" style="max-width: 800px; margin: 0 auto;">
    <div class="card-header">
        <h2 class="card-title">إضافة تذكرة طيران جديدة</h2>
    ...
    <form action="{{ route('admin.tickets.store') }}" method="POST">
        ...
        <div class="form-group" style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
            ...
            <div>
                ...
                <input type="text" id="weight" name="weight" value="{{ old('weight') }}" placeholder="مثال: 23 كجم">
            </div>
        </div>
        ...
        <div class="form-group" style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
            <div>
                ...
                <select id="trip_type" name="trip_type" required>
                    <option value="one_way" {{ old('trip_type', 'one_way') === 'one_way' ? 'selected' : '' }}>ذهاب فقط</option>
                    <option value="round_trip" {{ old('trip_type') === 'round_trip' ? 'selected' : '' }}>ذهاب وعودة</option>
                </select>
            </div>
            <div>
                <label for="price">سعر التذكرة (ج.م) *</label>
                <input type="number" id="price" name="price" value="{{ old('price') }}" step="0.01" min="0" required placeholder="مثال: 3200">
            </div>
        </div>
        ...
        <div class="form-group" style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
            ...
            <div>
                ...
                <input type="text" id="arrival_date" name="arrival_date" class="datetime-picker" value="{{ old('arrival_date') }}" readonly>
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <div class="duration-inputs">
                <div>
                    <input type="number" name="duration_days" value="{{ old('duration_days', 0) }}" min="0" placeholder="أيام">
                    <small>أيام</small>
                </div>
                <div>
                    <input type="number" name="duration_hours" value="{{ old('duration_hours', 0) }}" min="0" max="23" placeholder="ساعات">
                    <small>ساعات</small>
                </div>
                <div>
                    <input type="number" name="duration_minutes" value="{{ old('duration_minutes', 0) }}" min="0" max="59" placeholder="دقائق">
                    <small>دقائق</small>
                </div>
            </div>
        </div>
        ...
        <div id="return_trip_section" style="display: none;">
            ...
            <div class="form-group" style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
                ...
                <div>
                    ...
                    <input type="text" id="return_arrival_date" name="return_arrival_date" class="datetime-picker" value="{{ old('return_arrival_date') }}" readonly>
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <div class="duration-inputs">
                    <div>
                        <input type="number" name="return_duration_days" value="{{ old('return_duration_days', 0) }}" min="0" placeholder="أيام">
                        <small>أيام</small>
                    </div>
                    <div>
                        <input type="number" name="return_duration_hours" value="{{ old('return_duration_hours', 0) }}" min="0" max="23" placeholder="ساعات">
                        <small>ساعات</small>
                    </div>
                    <div>
                        <input type="number" name="return_duration_minutes" value="{{ old('return_duration_minutes', 0) }}" min="0" max="59" placeholder="دقائق">
                        <small>دقائق</small>
                    </div>
                </div>
            </div>
        ...
        <div class="form-group" style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px;">
            ...
            <div>
                ...
                <input type="number" id="number_of_children" name="number_of_children" value="{{ old('number_of_children', 0) }}" min="0">
            </div>
            <div>
                <label for="number_of_babies">الحد الأقصى للرضع</label>
                <input type="number" id="number_of_babies" name="number_of_babies" value="{{ old('number_of_babies', 0) }}" min="0">
            </div>
        </div>

        <div class="form-group">
            <label for="description">ملاحظات / وصف الرحلة (اختياري)</label>
            <textarea id="description" name="description" rows="4" placeholder="اكتب أي ملاحظات أو شروط خاصة بالرحلة (مثال: حقائب مجانية، طيران مباشر...)">{{ old('description') }}</textarea>
        </div>

        <button type="submit" class="btn btn-primary" style="width: 100%; padding: 12px; margin-top: 10px;">إضافة التذكرة</button>
    </form>
</div>
@endsection
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function updateReturnTripVisibility() {
        ...
        // arrival_date and return_arrival_date are optional but recommended
    }
